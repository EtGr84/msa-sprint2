...
ACL deny сохранен в `acl-deny.txt`: при `userid: test-user-3` и запросе `userId: test-user-2` возвращается пустой список (возврат ошибок вместо пустого списка требует предварительной проработки перечня ошибок и условий их возникновения, чего не было в ТЗ). 
